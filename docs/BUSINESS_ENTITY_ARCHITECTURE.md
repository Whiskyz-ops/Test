...
| **1** | ✅ **Shipped, all four.** India depreciation from `asset_blocks[]` (§2.4); disallowances s.40A(3)/40(a)/43B(h) folded into net profit (found and fixed along the way: `scope_payer_tds` and IN-23's own prior claim that this was "already flowing in" traced to dead code — `_evaluateSurchargeBuckets`'s calculator reads a top-level `business_income.expenses` object no rendered input ever writes to; also fixed a Phase-0 bug, employer PF/ESI deducted unconditionally instead of gated on s.43B(d)'s paid-before-due-date flag); F&O/speculative separation (§2.2), built as a genuine s.73/s.113 ring-fence rather than copying Layer 1's own live-preview calculator's unconditional netting; partner-firm pass-through (§2.3), remuneration + interest taxable, exempt profit share correctly excluded | No | Phase 0 |
