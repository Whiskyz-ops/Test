...
| XBR-6 | `computeIndiaItrForm` (ITR form determination) | ❌ | — | 🟢 | No reference. Small, rule-table-shaped function — good early pick once TAX-1/TAX-4 exist to feed it (they already do). **Scoped in detail 19 Jul 2026:** `computeIndiaItrForm(model, computed)` (computation.js:1679-1789, 111 lines) — ~12 disqualifier boolean checks + an entity-type cascade, all table-driven. Every input already ported except one to verify: `computed.indiaTax.grossTotalIncomeInr` — the DAG has `totalIncomeInrV3` (`in1-nodes-v3.js:365`), likely the same figure but needs a one-line semantic check (pre- vs. post-deduction) before assuming equality, not a new derivation either way. |
...
XBR-4, XBR-6, and CFL-7's still-blocked pieces.
...
| ❌ Not ported, no DAG reference | 16 (of which CFL-6 alone stands in for 48 individual findings) |
| **Total rows** | **40** |
...
- **Cross-border reconciliation (C):** the headline is now in-graph — XBR-1 (residency), XBR-2 (`computeFtc`, the product's headline FTC claim), and XBR-5 (apportionment) are all ✅, and `run-xborder-full.js` proves the net-unrelieved-double-tax figure end-to-end from raw form data. Remaining: XBR-3 (double-taxed-income map), XBR-4 (cross-basis), XBR-6 (ITR form) — all scoped, none blocked on each other or on what just closed.
...
5b. ~~**XBR-5 → XBR-6 → XBR-3 → XBR-4 → XBR-2, in that order**~~ **XBR-5 CLOSED 19 Jul 2026** (scoped and built same session — see the dated note in section C and XBR-5's own row) — `apportionment-nodes.js`, verified 132/132. **XBR-6 is next**, per the same order: it needs one field verified (`totalIncomeInrV3` vs. `grossTotalIncomeInr`), otherwise fully unblocked. Then XBR-3 (needs a set of US income breakouts verified — a first check while closing XBR-5 suggests `aggregateusincome-nodes.js`'s `aggregateUsIncomeResult` already computes the needed `foreign*` fields internally, just not yet exposed as named nodes; confirm before assuming this is free), then XBR-4 (two small raw leaves), then XBR-2 (three `usTaxResult` fields plus an explicit decision on the NRA path — TAX-8 is scoped out of the DAG entirely, so a first XBR-2 port is resident-taxpayer-only by necessity, which needs to be stated, not silently wrong). XBR-2 remains the highest-VALUE item (the product's headline FTC claim) even though it's no longer the correct STARTING point.
