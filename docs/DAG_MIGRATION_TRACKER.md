...
| AGG-5 | `aggregateAccounts` | 🟡 | `findings-batch5-nodes.js` (`aggregatePeakUsdResult`) | — | **Closed 19 Jul 2026** (from scratch, not the old `scope-nodes.js` boundary-read version — `bankAccountsRaw` reads raw `india.bank_accounts`/`us.bank_accounts`/`us.fbar_aggregate_peak_usd` directly, genuinely re-deriving `aggregatePeak`, not borrowing the engine's own). `fbar_limit`'s one real consumer, verified in `run-findings5.js`. Two fields remain unported: `bank_name`/`account_type` (the per-account display-label fields for the Schedule FA/accounts-list UI — no finding reads these). |
| AGG-6 | `aggregateTaxesPaid` (TDS/TCS/withholding totals feeding FTC) | 🟡 | `findings-batch5-nodes.js` (`taxesPaidUsResult`) | — | **Closed 19 Jul 2026**, US side only — `form67_required`'s one real consumer (`.us.total.usd`) needed just the 5 US withholding/estimated-tax raw fields, verified in `run-findings5.js`. The India side (advance tax by quarter, TDS/TCS) and the US prior-year-tax safe-harbor figure remain unported — no current finding reads them. |
...
|---|---|
| ✅ Ported & verified | 28 |
| 🟡 Partial (ported with recorded gaps) | 2 (AGG-5, AGG-6) |
| 🔶 Boundary-only (looks covered, isn't) | 1 |
| 📝 Explicitly scoped out (documented) | 2 |
| ❌ Not ported, no DAG reference | 7 |
| **Total rows** | **40** |
...
- **Conflict rule-book (E):** **100% by finding count per `audit:dag`'s own mechanical count (58 of 58) — CFL-1..6 are ALL closed.** Final tally: CFL-1..5's 5, the 8 `residency_status_*` findings (3 of which the audit script's regex-based scanner can't actually detect as engine-side IDs, since IN-40 constructs them via a ternary rather than a literal string first argument to `add()`, a pre-existing scanner imprecision not fixed here — the script still counts them correctly via the DAG-side mapping, just can't derive the engine-side "58 total" from source alone), and CFL-6's 48 across six batches (7+7+19+7+7+1). Only CFL-7 (the report-assembly layer — `buildFtcReport`, `buildTaxComputation`, etc., turning computed figures into `WISING.analyze()`'s exact return shape) remains unbuilt in this whole section — tracked in its own row above.
