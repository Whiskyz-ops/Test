...
| ❌ Not ported, no DAG reference | 14 (of which CFL-6 alone stands in for 48 individual findings) |
| **Total rows** | **40** |
...
- **Cross-border reconciliation (C):** the headline is now in-graph — XBR-1 (residency), XBR-2 (`computeFtc`, the product's headline FTC claim), XBR-3 (double-taxed-income map), XBR-5 (apportionment), and XBR-6 (ITR form) are all ✅, and `run-xborder-full.js` proves the net-unrelieved-double-tax figure end-to-end from raw form data. Remaining: XBR-4 (cross-basis) — the last row in this section, already scoped, not blocked on anything that just closed.
...
5b. ~~**XBR-5 → XBR-6 → XBR-3 → XBR-4 → XBR-2, in that order**~~ **XBR-5, XBR-2, XBR-6, AND XBR-3 all CLOSED 19 Jul 2026** — `apportionment-nodes.js` (132/132), `itrform-nodes.js` (91/91), and `doubletax-nodes.js` (55/55), this session; `ftc-nodes.js`/`xborder-full-nodes.js` (176/176 + 144/144), a parallel session — see the reconciliation note in section C for why closing out of the suggested order isn't a problem: XBR-2..6 are independent siblings. XBR-6's "one field to verify" turned out to be a real, small gap (`grossTotalIncomeInrV3`/`grossTotalIncomeInrCombined`, see its own row) — closed the same session it was flagged in, plus AGG-1's `agricultural_income_inr` knownMissing item as a natural side effect (XBR-6 is its one consumer). XBR-3's "needs verifying" turned out fully positive — `aggregateUsIncomeResult` already had every `foreign*` field it needed. **Remaining: XBR-4** — the last row in this cluster, needs two small raw leaves added (`model.assets.usOwns10PctForeignCorp`, `model.assets.usForeignCorps`), no other blockers per the same-day scoping pass.
