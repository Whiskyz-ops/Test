...
- **Conflict rule-book (E):** ~74% by finding count per `audit:dag`'s own mechanical count (43 of 58 — up from 24 before batch 3: CFL-1..5's 5, the 8 `residency_status_*` findings — 3 of which the audit script's regex-based scanner can't actually detect as engine-side IDs, since IN-40 constructs them via a ternary rather than a literal string first argument to `add()`, a pre-existing scanner imprecision not fixed here — CFL-6 batches 1/2/3's 7+7+19). CFL-6 alone still has 15 findings with no DAG reference, all individually scoped (see its own row) — mostly small, self-contained gaps (AGG-6/7/9, one TAX-1 metadata gap, LIM-1..6 gauges, one real new derivation for loss set-off).
