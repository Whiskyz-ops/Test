...
day** — see the four new §E-bis rows below. One of them (the India
DTAA-taint question) surfaced something bigger than a consistency-check
detail while stress-testing against the REAL `layer1_india.html` wizard
source (not just the engine, which is all prior rows in this doc were
checked against): `runResidencySolver()` in Layer 1 India is a full
~20-branch statutory determination — the 60/182-day tests, the 4-year
lookback, RNOR sub-status via the 9-of-10-years and 729-day-in-7-years
conditions, the employment/crew departure exception, the PIO/citizen visit
exception, and s.6(1A) deemed-residency (income >₹15L, not liable to tax
elsewhere) — and `normalize.js` reads NONE of it. Only the final locked
`final_india_residency_status` string crosses into the engine; every fact
that explains WHY it was set stays in the frontend, invisible to both
production and this DAG. That's a real, previously-undocumented gap (not
in `GAP_TRACKER.md` today, checked) — closer to that doc's remit (what
Layer 1 collects that the engine never reads) than this one's (engine vs.
DAG parity), and much larger in scope than a consistency check. Flagged
here, not yet acted on.
...
|---|---|---|---|
| `residency_status_understated_india` / `_overstated_india` / `_understated_us` / `_overstated_us` | `residency-nodes.js` (`residencyConsistencyFindings`) | 19 Jul 2026 | Closing XBR-1 surfaced a real gap: the engine stores day-count facts (`daysCurrentYear`, `sptMet`) right next to the trusted residency-status conclusion and never cross-checks them — `monitoring.js` will render a "210 of 182 days" progress bar next to a status that came from an entirely different source, with nothing to catch the contradiction. This checks two implications per country chosen specifically because they hold under every variant of the rule (days ≥182/183 but marked non-resident; days at 0 or below the 31-day SPT floor but marked resident) — not a re-derivation of residency status itself, which would need multi-year day-count history and qualitative facts (s.6(1A)'s 9-of-10-years lookback, SPT's 3-year weighted formula) this engine doesn't collect at all. Two of the four directions have a real, unmodeled legal exception (India s.6(1A) deemed-residency; US SPT exempt-individual visa status) and are worded as "verify," not "wrong," rather than overclaiming certainty. Individual-only on both sides at first pass — see the row below for the India company/HUF extension. Verified in `run-residency.js`: zero findings across all 11 real profiles (true-negative) plus 6 hand-built synthetic cases proving each direction fires exactly when it should. |
| `residency_status_understated_india_company` / `_overstated_india_entity` / `_understated_india_entity` | `residency-nodes.js` (`residencyConsistencyFindings`) | 19 Jul 2026 | Extension of the row above to the two India entity types the individual-only check deliberately gated out rather than covered: **company** (`residency_detail.is_indian_company` vs status — s.6(3) makes an Indian-incorporated company unconditionally resident, no POEM override, so `is_indian_company === true` + `NR` is exception-free wrong; the OTHER direction — foreign-incorporated but POEM resolves to ROR — is a genuinely different situation already covered by the real, unported `entity_dual_residency_poem` finding, conflicts.js:723, CFL-6, deliberately not duplicated here) and **HUF/firm/LLP/local authority/trust/AOP/BOI/AJP** (`residency_detail.is_wholly_outside_india` vs status — s.6(2)/s.6(4)'s binary "resident unless control & management wholly outside India" test, both directions exception-free, unlike the individual day-count check's lower-threshold ambiguity). US business entities (ccorp/scorp/partnership/trust) remain unchecked — `profile.incorporated_in_us`/`incorporation_state` exist in the raw data and one demo data point confirms `incorporated_in_us: true` ↔ `final_us_residency_status: "DOMESTIC_ENTITY"`, but the string Layer 1 records for a *foreign*-incorporated filer isn't yet confirmed — flagged as a follow-up, not guessed at. Verified in `run-residency.js`: the 3 real company/HUF profiles (already exercised by the existing all-11-profiles true-negative pass) plus 5 new synthetic cases — one per direction, plus two negative cases (foreign-incorporated+ROR must NOT fire, unanswered `is_wholly_outside_india` must NOT fire). 154/154 total. |
| `residency_status_understated_us_entity` / `_overstated_us_entity` | `residency-nodes.js` (`residencyConsistencyFindings`) | 19 Jul 2026 | Closes the US business-entity follow-up above — confirmed against the real source (`layer1_us.html:7101`) rather than guessed: `final_us_residency_status` for a ccorp/scorp/partnership/trust is only ever `"DOMESTIC_ENTITY"` or `"FOREIGN_ENTITY"`, so any non-`"DOMESTIC_ENTITY"` value is the foreign case. `profile.incorporated_in_us` vs `final_us_residency_status`, both directions exception-free — Layer 1's corporate short-circuit (`layer1_us.html:7099-7126`) sets the status purely from `incorporated_in_us` and returns immediately, before any of the individual SPT/DTAA/citizen logic runs, so there's no override path at all (unlike the India side — see the important caveat two rows below). Verified: `us_ccorp_indian_sub` (the one real entity profile) stays silent via the existing true-negative pass, plus 3 new synthetic cases. 157/157 total. |
| ⚠️ **Open question, not yet resolved: does `final_india_residency_status` mean pure domestic-law status, or can it be treaty-tainted?** | — | 19 Jul 2026 | Found while stress-testing the two rows above against the real `layer1_india.html` wizard (`runResidencySolver()`, ~L5717-5919), not assumed: the LIVE WIZARD overwrites `final_india_residency_status` to `"NR"` whenever `dtaa_treaty_residence === "us"` or `dtaa_forced_nr === true` — for EVERY entity type, applied unconditionally after the entity-type branching. But `computation.js`'s `resolveResidency()` (XBR-1, already ported) treats that same field as pure, treaty-INDEPENDENT domestic-law status: it derives `isResident`/`worldwide` from `status` alone, then applies the treaty "cedes worldwide taxation" consequence SEPARATELY (`indiaCedes`), never touching `status` or `isResident` itself. The demo fixture `dual_resident_h1b` matches the BACKEND's assumption (183 days, `final_india_residency_status: "ROR"`, `dtaa_treaty_residence: "us"`, all coexisting) — NOT what the live wizard would actually write if run today. This means either (a) the fixture is unrealistic and the live wizard's behavior is what production actually produces, in which case `residencyResult`/`resolveResidency`'s XBR-1 port has a latent correctness gap (a treaty-tied person's domestic-law `isResident` would read false, wrongly, since status arrives pre-collapsed to NR), or (b) the live wizard has the bug (conflating treaty tie-break with domestic status) and the fixture correctly reflects what a fixed wizard should produce. Left unresolved deliberately rather than guessed at — it changes what the three `understated_india*` consistency-finding rows above should assume: gate them against `dtaa_treaty_residence`/`dtaa_forced_nr` (if the wizard's current behavior is "correct" per this field's contract) or leave them as-is, since firing in this exact situation might be the MORE valuable behavior (surfacing the wizard's own conflation to the user) rather than a false positive to suppress. Not fixed in either direction pending that call. |
