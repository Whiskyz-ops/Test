...
- **Conflict rule-book (E):** ~86% by finding count per `audit:dag`'s own mechanical count (50 of 58 — up from 43 before batch 4: CFL-1..5's 5, the 8 `residency_status_*` findings — 3 of which the audit script's regex-based scanner can't actually detect as engine-side IDs, since IN-40 constructs them via a ternary rather than a literal string first argument to `add()`, a pre-existing scanner imprecision not fixed here — CFL-6 batches 1/2/3/4's 7+7+19+7). CFL-6 alone still has 8 findings with no DAG reference, all individually scoped (see its own row) — AGG-9 (equity comp), AGG-6 (withholding taxes-paid aggregate), LIM-1..6 (gauges), TAX-9 (state tax), and one dollar-impact recompute.
