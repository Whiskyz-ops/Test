...
|---|---|
| ✅ Ported & verified | 27 |
| 🟡 Partial (ported with recorded gaps) | 3 (AGG-5, AGG-6, CFL-6) |
...
- **Conflict rule-book (E):** ~98% by finding count per `audit:dag`'s own mechanical count (57 of 58 — up from 50 before batch 5: CFL-1..5's 5, the 8 `residency_status_*` findings — 3 of which the audit script's regex-based scanner can't actually detect as engine-side IDs, since IN-40 constructs them via a ternary rather than a literal string first argument to `add()`, a pre-existing scanner imprecision not fixed here — CFL-6 batches 1/2/3/4/5's 7+7+19+7+7). CFL-6 alone has exactly ONE finding left with no DAG reference: `holding_period_mismatch_`, which genuinely needs new architecture (a second `computeUsTax` pass), not just a new derivation.
