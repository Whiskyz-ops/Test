...
| XBR-3 | `mapDoubleTaxedIncome` (per-head doubly-taxed-income breakdown) | ❌ | — | 🟡 | No reference anywhere. **Scoped in detail 19 Jul 2026:** `mapDoubleTaxedIncome(model, residency)` (computation.js:1407-1444, 38 lines) — 6 declarative `pair()` calls comparing an India-side income head against a US-side "foreign-source counterpart" head, plus one reduce. India-side fields are all already-ported aggregation outputs (AGG-1 ✅). Needs verifying that `aggregateusincome-nodes.js` (AGG-3 ✅) exposes the specific US-side `foreign*` breakouts this reads (`foreignWages`, `foreignSelfEmployment`, `foreignRental`, `foreignInterest`, `foreignDividends`, `foreignCapitalGains` — narrower slices than the aggregate US totals AGG-3 already proved) before this is a pure port. |
| XBR-4 | `crossBasis` (income re-computed under the other country's code — the "same income, both codes" reconciliation table) | ❌ | — | 🟡 | No reference. **Correction, 19 Jul 2026: the "depends on XBR-2/TAX-9" claim above was checked against source and is wrong — removed.** `crossBasis(model, residency, usTax)` (computation.js:1514-1605, 92 lines) never reads `ftc` or `stateTax` at all; it's a sibling of XBR-2/XBR-5/XBR-6 in the orchestrator (`compute()`, computation.js:1794-1804), not downstream of any of them. Table-driven, 11 `row()` calls behind `if (amount > 0)` guards + 2 reduces, no new tax logic. Every income field it reads is already ported (AGG-1/AGG-3 ✅); `usTax.feie.appliedUsd` already available via the DAG's separate `feie` node (`ustax-nodes.js:159`). Needs two small raw-leaf reads not yet anywhere in the DAG: `model.assets.usOwns10PctForeignCorp`, `model.assets.usForeignCorps`. Effort re-tagged 🔴→🟡 given the above — it was overrated by the stale dependency claim. |
...
| ❌ Not ported, no DAG reference | 15 (of which CFL-6 alone stands in for 48 individual findings) |
| **Total rows** | **40** |

By subsystem, share of engine logic with zero DAG reference:
- **Income/deduction aggregation (A):** mostly closed — 3 rows ✅ plus AGG-1 🟡 (one specific side-channel read short of full, down from six), the rest are smaller withholding/equity-comp utility functions.
...
- **Cross-border reconciliation (C):** the headline is now in-graph — XBR-1 (residency), XBR-2 (`computeFtc`, the product's headline FTC claim), XBR-5 (apportionment), and XBR-6 (ITR form) are all ✅, and `run-xborder-full.js` proves the net-unrelieved-double-tax figure end-to-end from raw form data. Remaining: XBR-3 (double-taxed-income map), XBR-4 (cross-basis) — both scoped, neither blocked on each other or on what just closed.
...
5b. ~~**XBR-5 → XBR-6 → XBR-3 → XBR-4 → XBR-2, in that order**~~ **XBR-5, XBR-2, AND XBR-6 all CLOSED 19 Jul 2026** — `apportionment-nodes.js` (132/132) and `itrform-nodes.js` (91/91), this session; `ftc-nodes.js`/`xborder-full-nodes.js` (176/176 + 144/144), a parallel session — see the reconciliation note in section C for why closing out of the suggested order isn't a problem: XBR-2..6 are independent siblings. XBR-6's "one field to verify" turned out to be a real, small gap (`grossTotalIncomeInrV3`/`grossTotalIncomeInrCombined`, see its own row) — closed the same session it was flagged in, plus AGG-1's `agricultural_income_inr` knownMissing item as a natural side effect (XBR-6 is its one consumer). **Remaining: XBR-3 → XBR-4** — XBR-3 needs a set of US income breakouts verified (a first check while closing XBR-5 suggests `aggregateusincome-nodes.js`'s `aggregateUsIncomeResult` already computes the needed `foreign*` fields internally, just not yet exposed as named nodes — confirm before assuming this is free), XBR-4 needs two small raw leaves added.
