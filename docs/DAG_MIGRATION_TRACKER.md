...
| AGG-6 | ~~`aggregateTaxesPaid`~~ **Nearly fully closed 19 Jul 2026** | 🟡 | `findings-batch5-nodes.js` (`taxesPaidUsResult`), `report-batch1-nodes.js` (`taxesPaidIndiaResult`) | — | Batch 5 closed the US side (`form67_required`'s dependency). CFL-7 batch 1 closed the India side (advance tax by quarter, TDS/TCS — `buildDocuments`' `form_1116` trigger needed `.india.total.usd`). One field remains: `prior_year_total_tax_usd` (the Form 2210 100%/110%-of-prior-year safe-harbor figure) — no current DAG consumer. |
...
| LIM-2 | Form 8938 gauge | ❌ | — | 🟡 | No reference. Needs the "abroad" §911 test (already computed by `feieEligibility`, which TAX-6 ported) plus the 4-way MFJ/single × US-resident/abroad threshold table. No CFL-6 finding currently depends on this gauge, so not picked up alongside LIM-1/3/6. |
...
| LIM-4 | NRO-repatriation gauge | ❌ | — | 🟢 | No reference. No CFL-6 finding currently depends on this gauge. |
| LIM-5 | FEIE-exclusion-used gauge | ❌ | — | 🟢 | No reference, but the hard part (`feieEligibility`) is already ported (TAX-6) — this is close to a pure wiring exercise once picked up. No CFL-6 finding currently depends on this gauge. |
...
- **Limits & monitoring (D):** 3 of 7 gauges now ✅ (LIM-1/3/6, the three with a CFL-6 finding depending on them) — LIM-2/4/5 (no current finding needs them) and LIM-7 (`monitoring.js`'s pace projections) remain untouched.
