...
5b. ~~**XBR-5 → XBR-6 → XBR-3 → XBR-4 → XBR-2, in that order**~~ **XBR-5 AND XBR-2 both CLOSED 19 Jul 2026** — `apportionment-nodes.js` (132/132, this session) and `ftc-nodes.js`/`xborder-full-nodes.js` (176/176 + 144/144, a parallel session — see the reconciliation note in section C for why two closing out of the suggested order, at once, isn't a problem: XBR-2..6 are independent siblings). **Remaining: XBR-6 → XBR-3 → XBR-4**, roughly increasing in blocker size — XBR-6 needs one field verified (`totalIncomeInrV3` vs. `grossTotalIncomeInr`), XBR-3 needs a set of US income breakouts verified (a first check while closing XBR-5 suggests `aggregateusincome-nodes.js`'s `aggregateUsIncomeResult` already computes the needed `foreign*` fields internally, just not yet exposed as named nodes — confirm before assuming this is free), XBR-4 needs two small raw leaves added.
