...
function RegionCell({ r }) {
  return (
    <div className="flex items-center gap-2">
      {r.type === "country" ? <span className="text-base">{r.flag}</span> : <span className="text-[10px] font-black bg-navy/10 text-navy rounded px-1.5 py-0.5">{r.abbr}</span>}
...
function Tracker({ r }) {
  ...
  const over60 = pct >= 60;
  const color = over60 ? "#d97706" : "#16a34a";
  return (
    <div className="w-44">
      <div className="flex justify-between text-[10px] mb-1"><span className="text-muted">{pct}% to residency</span>{over60 && <span className="text-approaching font-bold">⚠ alert</span>}</div>
...
const YesNo = ({ v }) => <span className={v ? "text-accent font-semibold" : "text-muted"}>{v ? "Yes" : "No"}</span>;
...
export default function DetailTable({ category, regions }) {
  ...
  if (category === STATUS.EXPOSED) {
    ...
    row = (r) => <tr key={r.id} className="border-t border-line hover:bg-surface2"><TD><RegionCell r={r} /></TD><TD><StatusBadge status={r.status} /></TD><TD right mono><span className="text-exposed font-semibold">{fmtUsd(r.estimatedTaxUsd)}</span></TD><TD>{r.triggerDate || "—"}</TD><TD right mono>{fmtUsd(r.incomeExposedUsd)}</TD></tr>;
    ...
    row = (r) => <tr key={r.id} className="border-t border-line hover:bg-surface2"><TD><RegionCell r={r} /></TD><TD><Tracker r={r} /></TD><TD mono><Days r={r} /></TD><TD mono><Reporting r={r} /></TD><TD><YesNo v={r.physicalPresence} /></TD></tr>;
    ...
    row = (r) => <tr key={r.id} className="border-t border-line hover:bg-surface2"><TD><RegionCell r={r} /></TD><TD>{r.triggerDate || "—"}</TD><TD><span className="text-body">{r.reason || "Filing / disclosure only"}</span></TD><TD mono><Days r={r} /></TD><TD><YesNo v={r.physicalPresence} /></TD></tr>;
  } else {
    head = <tr><TH>Region</TH><TH>Status</TH><TH right>Est. Tax</TH><TH>Days Present</TH><TH>Physical Presence</TH></tr>;
    row = (r) => <tr key={r.id} className="border-t border-line hover:bg-surface2"><TD><RegionCell r={r} /></TD><TD><StatusBadge status={r.status} /></TD><TD right mono>{r.estimatedTaxUsd ? fmtUsd(r.estimatedTaxUsd) : "—"}</TD><TD mono><Days r={r} /></TD><TD><YesNo v={r.physicalPresence} /></TD></tr>;
  }
  return (
    <div className="overflow-x-auto rounded-2xl border border-line bg-surface shadow-card">
      <table className="w-full"><thead className="bg-surface2">{head}</thead><tbody>{regions.map(row)}</tbody></table>
    </div>
  );
}
