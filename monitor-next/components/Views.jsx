...
const SEV = { critical: "#ef4d6a", warning: "#d97706", info: "#6d5ef7" };
const GREEN = "#16a34a";
...
const Ref = ({ children }) => <span className="text-[9px] font-bold uppercase tracking-wide px-2 py-0.5 rounded bg-surface2 border border-line text-muted">{children}</span>;
        ...
        {chips.map(([id, label]) => {
          ...
          return (
            <button key={id} onClick={() => setFilter(id)}
              className={"px-2.5 py-1 rounded-md text-[11px] font-bold border transition-colors " + (on ? "bg-head text-white border-head" : "bg-surface border-line text-body hover:border-accent/40")}>
        ...
        {shown.map((f) => {
          ...
          return (
            <div key={f.id} className="rounded-lg bg-surface border border-line shadow-card overflow-hidden" style={{ borderLeft: `3px solid ${SEV[f.severity]}` }}>
              <button onClick={() => setOpen(isOpen ? null : f.id)} className="w-full flex items-center gap-3 px-3 py-2.5 text-left hover:bg-surface2">
              ...
              {isOpen && (
                <div className="px-3 pb-3 pt-0">
                  ...
                  <div className="text-[12px] text-head mt-2 leading-relaxed"><span className="font-bold" style={{ color: GREEN }}>▸ Action:</span> {f.recommendation}</div>
...
export function ResidencyView({ result }) {
  ...
  const stCol = { resident: "#ef4d6a", will_flip: "#d97706", safe: "#16a34a" };
  const Flag = ({ label, s }) => (
  ...
  const treatyRow = (label, ok, okText, badText) => (
    <div className="flex items-center justify-between py-1.5 border-b border-line text-[12px]">
      ...
      <span className="font-semibold" style={{ color: ok ? GREEN : "#ef4d6a" }}>{ok ? okText : badText}</span>
    </div>
  );
  return (
    <div className="space-y-6">
      {r.dualResident && <div className="rounded-xl border border-exposed/30 bg-exposed/10 p-3 text-[13px] font-semibold" style={{ color: "#c81e46" }}>⚠ Dual tax residency — resolve the India-US DTAA Article 4 tie-breaker.</div>}
          ...
          {(mon ? mon.residency : []).map((c, i) => {
            ...
            return (
              <div key={i}>
                ...
                <div className="h-2 rounded-full bg-line overflow-hidden"><div className="h-full rounded-full" style={{ width: pct + "%", background: stCol[c.status] }} /></div>
...
export function FilingsView({ result }) {
  ...
  const jColor = { US: "#6d5ef7", IN: "#b45309" };
  return (
    <div className="space-y-6">
      <Card title="Compliance Calendar" sub="Filing & payment deadlines with countdowns">
        <div className="space-y-1.5">
          {upcoming.concat(passed).map((x, i) => {
            ...
            const dueColor = x.status === "due_soon" ? "#d97706" : (isPast ? "#9ca3af" : GREEN);
            return (
              <div key={i} className={"flex items-center gap-3 p-2 rounded-lg " + (isPast ? "opacity-45" : "bg-surface2")}>
                <span className="text-[9px] font-black px-2 py-0.5 rounded" style={{ background: jColor[x.jur] + "1f", color: jColor[x.jur] }}>{x.jur}</span>
  ...
  const Block = ({ block }) => (
    <div className="mb-2"><div className="text-[11px] font-bold text-body mb-2">{block.title}</div>
      <div className="space-y-1">{block.rows.map((r, i) => {
        const c = r.warn ? "#ef4d6a" : r.emphasis ? GREEN : "#413d54";
        return <div key={i} className={"flex justify-between text-[12px] " + (r.emphasis || r.warn ? "font-bold" : "")}><span style={{ color: r.emphasis || r.warn ? c : "#413d54" }}>{r.label}</span><span className="font-mono" style={{ color: c }}>{fmtUsd(r.usd)}</span></div>;
      })}</div></div>
  );
  return (
    <Card title="FTC Reconciliation">
      <div className={"rounded-xl p-3 mb-4 border " + (net > 0 ? "border-exposed/30 bg-exposed/10" : "border-emerald-500/30 bg-emerald-50")}>
        <div className="text-[10px] uppercase tracking-widest text-muted">Net unrelieved double tax</div>
        <div className="font-display font-extrabold text-2xl" style={{ color: net > 0 ? "#ef4d6a" : GREEN }}>{fmtUsd(net)}</div>
      </div>
...
function TaxCard({ taxComputation, fxRate }) {
  const Block = ({ block, isInr, accent }) => (
    <div className="rounded-xl p-3" style={{ background: accent + "0d", border: `1px solid ${accent}26` }}>
      <div className="flex items-center justify-between mb-2"><div className="text-[12px] font-bold text-head">{block.title}</div><span className="text-[9px] font-bold uppercase px-2 py-0.5 rounded bg-surface border border-line text-muted">eff {Math.round(block.effectiveRate * 100)}%</span></div>
      <div className="space-y-1">{block.rows.map((r, i) => {
  ...
  return (
    <Card title="Tax Computation" sub="Planning-grade, from Layer 1">
      <Block block={taxComputation.india} isInr accent="#b45309" />
      <div className="h-3" />
      <Block block={taxComputation.us} accent="#6d5ef7" />
    </Card>
  );
}
  ...
  return (
    <Card title="Documents to File" sub={req + " required · triggered by this taxpayer's cross-border facts"}>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        {docs.map((d) => (
          <div key={d.id} className={"flex items-start gap-3 p-3 rounded-lg " + (d.required ? "bg-surface2 border border-line" : "opacity-45")}>
            <span className="text-[9px] font-black px-2 py-0.5 rounded mt-0.5" style={{ background: jColor[d.jurisdiction] + "1f", color: jColor[d.jurisdiction] }}>{d.jurisdiction}</span>
            <div className="flex-1">
              <div className="text-[12px] font-bold text-head flex items-center gap-2">{d.name}{d.required ? <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-exposed/15" style={{ color: "#c81e46" }}>Required</span> : <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-surface2 text-muted">N/A</span>}</div>
...
export function AccountsView({ result }) {
  ...
  const color = { ok: "#16a34a", approaching: "#d97706", breached: "#ef4d6a" };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <Card title="Reporting Limits" sub="FBAR · FATCA 8938 · LRS · FEIE">
        <div className="space-y-4">
          {result.computed.limits.map((g) => {
            ...
            return (
              <div key={g.id}>
                <div className="flex justify-between text-[11px] mb-1"><span className="text-body font-semibold">{g.label}{g.status === "breached" && <span className="ml-1 text-[9px] font-bold px-1.5 py-0.5 rounded bg-exposed/15" style={{ color: "#c81e46" }}>BREACHED</span>}</span><span className="font-mono" style={{ color: color[g.status] }}>{Math.round(g.pct * 100)}%</span></div>
                <div className="h-2 rounded-full bg-line overflow-hidden"><div className="h-full rounded-full" style={{ width: pct + "%", background: color[g.status] }} /></div>
      ...
      <Card title="Foreign Accounts" sub={accts.length + " account(s) · drives FBAR / Schedule FA"}>
        {accts.length === 0 ? <Empty>No foreign accounts on file.</Empty> : (
          <div className="space-y-1.5">
            {accts.map((a, i) => (
              <div key={i} className="flex items-center gap-3 p-2 rounded-lg bg-surface2">
...
export function ClientsView({ clients, activeId, onPick }) {
  ...
  const Kpi = ({ label, value, accent, sub }) => (
    <div className="rounded-2xl bg-surface border border-line shadow-card p-4">
      ...
      <div className="font-display font-extrabold text-2xl" style={{ color: accent || "#191527" }}>{value}</div>
      {sub && <div className="text-[11px] text-muted mt-1">{sub}</div>}
    </div>
  );
  const healthColor = (h) => (h >= 80 ? "#16a34a" : h >= 50 ? "#d97706" : "#ef4d6a");
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <Kpi label="At risk" value={atRisk} accent={atRisk ? "#ef4d6a" : "#16a34a"} sub="health < 50" />
        <Kpi label="Open critical" value={openCritical} accent={openCritical ? "#ef4d6a" : "#16a34a"} sub="conflicts" />
        <Kpi label="Combined tax" value={fmtUsd(totalTax)} sub="IN + US, all clients" />
        <Kpi label="Residual double tax" value={fmtUsd(totalResidual)} accent={totalResidual ? "#ef4d6a" : "#16a34a"} sub="unrelieved" />
      </div>
      <div className="overflow-x-auto rounded-2xl border border-line bg-surface shadow-card">
        <table className="w-full">
          ...
          <tbody>
            {sorted.map((c) => (
              <tr key={c.id} onClick={() => onPick(c.id)} className={"border-t border-line cursor-pointer hover:bg-surface2 " + (activeId === c.id ? "bg-accentSoft" : "")}>
                ...
                <td className="px-4 py-3"><span className="text-[9px] font-bold uppercase px-2 py-0.5 rounded" style={{ background: c.isBusiness ? "#8b5cf61f" : "#6d5ef71f", color: c.isBusiness ? "#6d28d9" : "#4f46e5" }}>{c.isBusiness ? "Business" : "Individual"}</span></td>
                <td className="px-4 py-3 text-[12px] text-body">{(c.indiaStatus || "—")}<span className="text-muted"> / </span>{(c.usStatus ? c.usStatus.replace(/_/g, " ") : "—")}{c.dualResident && <span className="ml-1 text-[9px] font-bold px-1 py-0.5 rounded bg-exposed/15" style={{ color: "#c81e46" }}>DUAL</span>}</td>
                <td className="px-4 py-3 text-right font-mono text-[12px] text-head">{fmtUsd(c.combinedTaxUsd)}</td>
                <td className="px-4 py-3 text-right font-mono text-[12px]" style={{ color: c.netDoubleTaxUsd > 0 ? "#ef4d6a" : "#9ca3af" }}>{fmtUsd(c.netDoubleTaxUsd)}</td>
                <td className="px-4 py-3 text-[12px]"><span className="font-bold" style={{ color: "#ef4d6a" }}>{c.critical}</span><span className="text-muted"> · </span><span style={{ color: "#d97706" }}>{c.warning}</span></td>
                <td className="px-4 py-3"><div className="flex items-center gap-2"><div className="w-16 h-1.5 rounded-full bg-line overflow-hidden"><div className="h-full rounded-full" style={{ width: Math.max(4, c.healthScore) + "%", background: healthColor(c.healthScore) }} /></div><span className="text-[11px] font-mono" style={{ color: healthColor(c.healthScore) }}>{c.healthScore}</span></div></td>
  ...
  return (
    <Card title="Integrations" sub="Connected data sources feed the engine daily">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {rows.map((r, i) => (
          <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-surface2 border border-line">
            ...
            {r.connected
              ? <span className="text-[10px] font-bold px-2 py-1 rounded-lg flex items-center gap-1" style={{ background: "#16a34a1a", color: "#15803d" }}><span className="w-1.5 h-1.5 rounded-full" style={{ background: "#16a34a" }} /> Connected</span>
              : <span className="text-[10px] font-bold px-2 py-1 rounded-lg bg-surface text-body border border-line">Connect</span>}
          </div>
        ))}
      </div>
    </Card>
